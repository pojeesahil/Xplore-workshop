...
while choice =='y' or choice =='Y' : # make 'Y' valid too
    try:
        ...
        numbers =list(map(int,input("Enter the input numbers separated by spaces: ")))
        ...
        if len(numbers) != len(operators)-1: # this seems odd... u might say it's ... off by one
            print("Number of operators must be less than number of numbers.") # replace wiht better message :)
            continue
...
# can you make the code shorter and with improved answer? yess
# like handling any basic arithmetic equation (that may have brackets too) ?
# u might wanna find a special function in python the function named eval()
